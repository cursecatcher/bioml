...
# import dataset as ds, clustering as clu, utils 
...
class DataRuleSet(ds.BinaryClfDataset):
    ...
    def __init__(self, io, rules = None, target_cov: str = None, pos_labels: tuple = ..., neg_labels: tuple = ...):
        ...
        if rules is not None:
            # is a filename - need to be loaded 
            rs = isinstance(rules, str)
            # list of rules ( i hope )
            rl =  isinstance(rules, list)
    ...
    def bcd(self) -> ds.BinaryClfDataset:
        ...
        bds = ds.BinaryClfDataset(df, target_cov=self.TARGET, pos_labels="1", neg_labels="0")
    ...
    def add_rule(self, rule: CompositeRule):        
        srule = str(rule)

        if srule not in self.rules:
            bisect.insort(self.rules, srule)

            index2one = self.data.query( rule.str_eval() ).index 
    ...
    def phi_correlation_rules(self, ax = None):
        rules = list( self )
        nr = len(rules)
        ...
        sns.heatmap(matrix, mask=mask, vmin=-1, vmax=+1, xticklabels=signatures, yticklabels=signatures, ax=ax)

        # utils.plt.show()


    def phi_correlation_target(self):
        elems = list() 
    ...
    def extract_subrules(self, ruleset: ds.FeatureList):

        new_rd = self.copy() #copy original data
        new_rd.ruledata = new_rd.ruledata[ [new_rd.TARGET] + ruleset ]  # copy selected rules 
...
class RuleSelector:
    ...
    def selection(self, dataset: DataRuleSet, max_rules: int = None) -> list:
        candidates = dict()
        ...
        return selected_rules.index.tolist()  #self.__selected.index.tolist()
